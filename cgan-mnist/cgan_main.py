...
from cgan.config import lr, bs as batch_size
from cgan.load_mnist import data_loader
from cgan.discriminator import Discriminator
from cgan.generator import CNNGenerator as Generator
...
writer = SummaryWriter(logdir=f'../runs/mnist_cgan_{now()}:cnn2:lr={lr}:bs={batch_size}')
...
for epoch in range(num_epochs):
    ...
    for i, (images, labels) in enumerate(data_loader):
        ...
        if step % display_step == 0:
            ...
            labels = torch.LongTensor(torch.cat([torch.arange(10)]*10)).cuda()
